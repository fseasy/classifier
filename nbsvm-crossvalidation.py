...
def build_doc_info(labels , docs_l) :
    ...
    return > list [ {} , ...] , every element like {label , [  ]  } , where label is the doc label , and list in it is the words represent the doc , and has no redundancy , that means , no dulplicate words
    '''
    doc_info = []
    for label , docs in zip(labels , docs_l) :
        for doc in docs :
            doc = list(set(doc))
            doc_info.append({'label':label , 'doc':doc})
    return doc_info
...
def build_dict_for_k_folds(doc_folds , exclude_idx) :
    ...
    for i in range(len(doc_folds)) :
        if i == exclude_idx :
            continue
        docs = doc_folds[i]
        for doc_dict in docs : # [ {'label': , 'doc':[] } ]
...
def main(pos , neg , ngram , cv_k) :
    ...
    k_fold_cross_validation(cv_k , doc_info)
...
if __name__ == "__main__" :
    ...
    parser.add_argument("--liblinear" , default="/home/xx/bin/liblinear-1.96/python" , type=str)
